...
11/04/2018:
    *_CA333m.pkl: Use 333mMLay V-4.10 data with MODIS colocated data. Other *.pkl s use 5km CALIOP data.
    *_CA333m_v2.pkl: Use 5km Layer data to detect "single layer water clouds" and use 333m opacity flag.
11/09/2018:
    *_CA333m_v3.pkl: Use 1km layer data to identify "single layer water clouds" and use 333m opacity flag.
    Opacity qualtiy is defined. See the table in the comments.
11/13/2018:
    *_CA333m_v4.pkl: Uses 5km Layer data to identify "single layer water clouds" and use 333m opacity flag
11/16/2018:
    *_CA1km_v1.pkl: Both opacity and water cloud detections from 1km
11/07/2024:
    started making changes to work with Adeleke
"""
...
from cpnMODISlib import SDSinfo, dsetMOD021K, drawrgb, drawContourf
from cpnCALCATSlib import dsetCALCATS,Extract_Feature_Info
from spotlib import savefig, find_CDF, save_obj, mapProject, add_cb
...
def getCOslwcld_from_CA1km(sd,colFlag):
    ...
    if wcld_ix.shape[0]==colFlagEQ1km_feature.shape[0]:
        ...
        COslwcld1km=COslwcld1km_feature.any()
        col_mismatch=0
    else:
        col_mismatch=1
        COslwcld1km,COslwcld1km_feature=np.nan,np.nan
        print('collocation mismatch')
...
def getCOslwcld_from_CA5km(sd,colFlag):
    ...
    vfm = sd.select('Feature_Classification_Flags').get()
    nly = sd.select('Number_Layers_Found').get()
    feature_type, feature_type_qa,ice_water_phase, ice_water_phase_qa,feature_subtype,\
    cloud_aerosol_psc_type_qa,horizontal_averaging=Extract_Feature_Info(vfm,nly)
    nlayers=feature_type.shape[1]
    colFlagEQ5km=colFlag.reshape((5,colFlag.size/5)).sum(axis=0)>4#all 5 1km MODIS-CALIOP successful collocation
    colFlagEQ5km_feature=np.repeat(colFlagEQ5km,nlayers).reshape(colFlagEQ5km.size,nlayers)#successful collocations
    wcld_ix=(feature_type==2)*(ice_water_phase==2)
    single_layers=np.repeat(nly.squeeze()==1,nlayers).reshape(nly.size,nlayers)#single layer detections
    COslwcld5km_feature=single_layers*wcld_ix*colFlagEQ5km_feature#Colocated single layer water cloud
    COslwcld5km=COslwcld5km_feature.any(True)
    return COslwcld5km,COslwcld5km_feature
def getCOslwcld_from_CA333(CA333m,cFlag):
    '''
    To get colocated single-layer water clouds from 333m CALIOP data and aggregate
    into 1km.
    -------------
    cFlag:boolean array
    '''
    ...
    feature_type, feature_type_qa,ice_water_phase, ice_water_phase_qa,feature_subtype, cloud_aerosol_psc_type_qa,horizontal_averaging=Extract_Feature_Info(vfm,nly)
    wcld_ix=(feature_type==2)*(ice_water_phase==2)
    colFlagEQ333m=cFlag.repeat(3)#A successful colocated data point equivalent to 3 333m points
    colFlagEQ333m_feature=np.repeat(colFlagEQ333m,feature_type.shape[1]).reshape(colFlagEQ333m.size,feature_type.shape[1])#successful collocations
    single_layers=np.repeat(nly.squeeze()==1,feature_type.shape[1]).reshape(nly.size,feature_type.shape[1])#single layer detections
    COslwcld=single_layers*wcld_ix*colFlagEQ333m_feature#Colocated single layer water cloud
    COslwcld_column=COslwcld.any(True)
    COslwcld1km=COslwcld_column.reshape((3,COslwcld_column.size/3)).sum(axis=0)>2# Select if all neighboring 3 333m observations are single layer water clouds
...
def opacity_from333m(CA333m,COslwcld333m_feature):
    ...
    |_________|_________________________|_____________________|
    '''
    op1km1=op_333m.reshape((3,op_333m.size/3)).sum(axis=0)>2 #All 3 333m lidar shots see opaque cloud layer
    op1km2=op_333m.reshape((3,op_333m.size/3)).sum(axis=0)>1 # 2/3 333m lidar shots see opaque cloud layer
    op1km3=op_333m.reshape((3,op_333m.size/3)).sum(axis=0)>0 # 1/3 333m lidar shots see opaque cloud layer
    tra1k1=tra333m.reshape((3,tra333m.size/3)).sum(axis=0)>2 #All 3 333m lidar shots see non-opaque cloud layers
    tra1k2=tra333m.reshape((3,tra333m.size/3)).sum(axis=0)>1 # 2/3 333m lidar shots see non-opaque cloud layers
    tra1k3=tra333m.reshape((3,tra333m.size/3)).sum(axis=0)>0 # 1/3 333m lidar shots see non-opaque cloud layers
...
def plot_CODHist(modCOD,temp_stamp,ax1,tps,normed=True,bins=None):
    ...
    '''
    if bins==None: bins=np.logspace(np.log10(0.01),np.log10(151.0), 100)
    fig1_ttl=temp_stamp+'_single_layer_wcld_CoLCALIPSO-MODIS_COD'
    f0,xe0,_=ax1.hist(modCOD[tps[0]][~np.isnan(modCOD[tps[0]])],density=normed,histtype='step',color='b',linewidth=2.0,bins=bins,label=tps[0])
    f1,xe1,_=ax1.hist(modCOD[tps[1]][~np.isnan(modCOD[tps[1]])],density=normed,histtype='step',color='k',linewidth=2.0,bins=bins,label=tps[1])
    f2,xe2,_=ax1.hist(modCOD[tps[2]][~np.isnan(modCOD[tps[2]])],density=normed,histtype='step',color='r',linewidth=2.0,bins=bins,label=tps[2])
#    ax1.hist(modCOD['lowSun'][~np.isnan(modCOD['lowSun'])],normed=True,histtype='step',color='g',linewidth=2.0,bins=bins,label='Low Sun')
#    ax1.hist(modCOD['highSn'][~np.isnan(modCOD['highSn'])],normed=True,histtype='step',color='c',linewidth=2.0,bins=bins,label='High Sun')
    ...
    if normed:
        ax1.set_ylabel('PDF')
        ax1.text(0.6,0.75,'md,mn,std: %0.1f, %0.1f, %0.1f'%(find_mode(f2,xe2),np.nanmean(modCOD[tps[2]]),\
                                                np.nanstd(modCOD[tps[2]])),transform = ax1.transAxes,color='r')
...
def plot_CODCDF(modCOD,temp_stamp,tps=['calopq','allang','calTra','TraLow','TraHig'],bins=np.logspace(np.log10(0.01),np.log10(151.0), 100)):
    ...
    COD CDF
    '''
    b0,c0=find_CDF(modCOD[tps[0]][~np.isnan(modCOD[tps[0]])],bins=bins)
    b1,c1=find_CDF(modCOD[tps[1]][~np.isnan(modCOD[tps[1]])],bins=bins)
    b2,c2=find_CDF(modCOD[tps[2]][~np.isnan(modCOD[tps[2]])],bins=bins)
    b3,c3=find_CDF(modCOD[tps[3]][~np.isnan(modCOD[tps[3]])],bins=bins)
    b4,c4=find_CDF(modCOD[tps[4]][~np.isnan(modCOD[tps[4]])],bins=bins)
...
def find_pot_3Deffects(show_3Df=True,year='2010',end_m=12,end_d=31):
    ...
    colFil=[] #time to get filename in collocated dataset
    modCOD={'allang':[]}
    calCOD={'allang':[]}
    calOpa={'Hig':[],'Med':[],'Low':[]}
    calTra={'Hig':[],'Med':[],'Low':[]}
    ...
    for day in np.arange(total_days):
        d=start_date+datetime.timedelta(int(day))
        yc='%04i'%d.year
        mc='%02i'%d.month
        dc='%02i'%d.day
        date_char=yc+'-'+mc+'-'+dc
        ...
        for f in os.listdir(indexP):
            if fnmatch.fnmatch(f,fn_pattern):
                ...
                if COslwcld1km:
                    colFil = np.append(colFil,datetime.datetime.strptime(f.split('.')[1][:-5], "%Y-%m-%dT%H-%M-%SZ"))
                    modSZA['allang']=np.append(modSZA['allang'],sza)
                    modCOD['allang']=np.append(modCOD['allang'],COD) 
                    locs['lat']=np.append(locs['lat'],mod_lat)
                    locs['lon']=np.append(locs['lon'],mod_lon)
                
                #Opacity Flag
                #=============
                
                if op_from=='1km':
                    op1km,tra1k=opacity_from1km(CAL1km,COslwcld1km_feature)
                    calOpa['Hig']=np.append(calOpa['Hig'],op1km[COslwcld1km])
                    calTra['Hig']=np.append(calTra['Hig'],tra1k[COslwcld1km])
                    if np.any((COD>30)*tra1k):
                        print("CALIOP transparent mod_COD>30: %s"%(f))
                        print(np.where((COD>30)*tra1k))
                elif op_from=='333m':
                    op1km1,op1km2,op1km3,tra1k1,tra1k2,tra1k3=opacity_from333m(CA333m,COslwcld333m_feature);CA333m.end() 
                    calOpa['Hig']=np.append(calOpa['Hig'],op1km1[COslwcld1km])
                    calOpa['Med']=np.append(calOpa['Med'],op1km2[COslwcld1km])
                    calOpa['Low']=np.append(calOpa['Low'],op1km3[COslwcld1km])
                    calTra['Hig']=np.append(calTra['Hig'],tra1k1[COslwcld1km])
                    calTra['Med']=np.append(calTra['Med'],tra1k2[COslwcld1km])
                    calTra['Low']=np.append(calTra['Low'],tra1k3[COslwcld1km])
...
#                pilmax=Use_CALIOP5km(year,f,calCOD,modCOD,calOpa,pt_ill,pilmax,pe_names,COslwcld1km,lowSun,highSn,colFlag,COD,show_3Df,mod_lon,mod_lat)
    ...
    obj={'modCOD':modCOD,'calCOD':calCOD,'locs':locs,'modCOD':modCOD,'calOpa':calOpa,'calTra':calTra,'modSZA':modSZA,'colFil':colFil}
    save_obj(obj,temp_stamp+version_append)
...
#    ctf2 = ax5.contour(COT_mesh,SZA_mesh,Ntot.T/Ntot.max(),8,linewidths=1.0,colors='k')
    ...
    add_cb(fig5,ctf1,ax5,ticks=np.arange(0,0.251,0.05),label='CALIOP Transparent Cloud Fraction',orientation='vertical')
#    ax5.clabel(ctf2, inline=1, fontsize=10)
    fig5.suptitle(fig5_ttl)
    fig5.tight_layout(rect=[0,0,1,0.97])
    savefig(fig5,fig5_ttl,'figures/') 
